...
(function(){
	...
	function call_remote_async(url, pack, defer){
		pack.data = JSON.stringify(pack.data);
		webix.ajax().post(url, pack).then(function(text){
	...
	function call_remote_sync(url, pack, args){
		...
		pack.data = JSON.stringify(pack.data);
		return parse_responce( webix.ajax().sync().post(url, pack).responseText ).data;
	}
...
webix.ajax = function(url,params,call,master){
	...
	if (arguments.length!==0)
		(new webix.ajax()).get(url,params,call,master);
...
webix.ajax.prototype={
	...
	*/
	_send:function(url,params,call, master, mode){
		...
		if (!webix.callEvent("onBeforeAjax", [mode, url, params, x])) return;
...
if (!webix.ui){
	ui = webix.ui = function(config, parent, id){
		...
		if (top_node._settings && top_node._settings._hidden){
...
webix.ui._detectScrollSize = function(){
	...
	var skin = { 110:"air", 120:"aircompact", 130:"clouds", 140:"web", 150:"terrace", 160:"metro", 170:"light", 180:"glamour", 190:"touch", 200:"flat" , 210:"compact"}[Math.floor(div.offsetHeight/10)*10];
...
webix.ui.each = function(parent, logic, master){
	if (parent){
...
webix.protoUI({
	...
	removeView:function(id){
		...
		if (target >= 0){
			...
			if (form && view.name)
				delete form.getCleanValues()[view.name];
...
webix.protoUI({
	...
	_parse_cells:function(){
		...
		if (this._padding && this._margin > 0 && !this._settings.borderless){
		 	this._contentobj.style.borderWidth="1px";
	...
	$getSize:function(dx, dy){
		...
		if (this._render_borders || (this._padding && this._margin>0)){
			var _borders = this._settings._inner;
			if (_borders){
				dx += (_borders.left?0:1)+(_borders.right?0:1);
				dy += (_borders.top?0:1)+(_borders.bottom?0:1);
	...
	_beforeResetBorders:function(collection){
		if (this._padding && this._margin && !this._settings.borderless){
			for (var i=0; i < collection.length; i++){
...
webix.protoUI({
	...
	tooltip_setter: function(value){
		if(this.$view.firstChild)
			this.$view.firstChild.title = value;
		return value;
	},
	type_setter:function(value){
	...
	_types:{
		...
		iconButtonTop:"<button type='button' class='webix_img_btn_abs webix_img_btn_abs_top webixtype_base' style='width:100%;top:0px;text-align:center;'><span class='webix_icon fa-#icon#'></span><div class='webix_img_btn_text'>#label#</button></div>"
...
webix.protoUI({
	...
	defaults:{
		template:"<button type='button' style='height:100%;width:100%' class='webix_icon_button'><span class='webix_icon fa-#icon# '></span></button>",
		width:33
	},
	_set_inner_size:function(){
...
webix.protoUI({
	...
	_get_input_width: function(config){
		var width = (this._input_width||0)-(config.label?this._settings.labelWidth:0)-webix.skin.$active.inputPadding*2 - (config.iconWidth || 0);
	...
	_render_div_block:function(obj, common){
		...
		var text = (obj.text||obj.value||(obj.placeholder?"<span class='webix_placeholder'>"+obj.placeholder+"</span>":""));
	...
	$renderInput: function(config, div_start, id) {
		...
		if(div_start){
			...
			html += this._baseInputHTML("input")+"id='" + id + "' type='"+(config.type||this.name)+"' value='" + (config.value||"") + "' style='width: " + width + "px; text-align: " + inputAlign + ";'";
			var attrs = this._settings.attributes;
			if (attrs)
				for(var prop in attrs)
					html += " "+prop+"='"+attrs[prop]+"'";
	...
	defaults:{
		template:function(obj, common){
			...
			for(var i=0; i<options.length; i++){
				html+="<button style='width:"+(options[i].width || optionWidth)+"px' ";
		...
		if (this._settings.options){
			...
			for (var i=0; i < this._settings.options.length; i++)
				if (this._settings.vertical || this._settings.options[i].newline)
					count++;
			size[3] = size[2] = Math.max(size[2], (this.config.optionHeight||25) * count);
		}
		return size;
	},
...
webix.protoUI({
	...
	$init:function(){
		this.attachEvent("onBlur", webix.bind(function(e){
			...
			if (data && !(this.getInputNode().value==="" && webix.$$(this.config.suggest).getList().getItem(data).value!==""))
				this.$setValue(data);
...
webix.protoUI({
	...
	_init_popup:function(){ 
		...
		if (obj.suggest)
			...
			obj.popup = obj.suggest = this.suggest_setter({
				type:"calendar", height:240+(timepicker?30:0), width:250,
	...
	$setValue:function(value){
		...
		this.getInputNode().innerHTML = this._settings.text || this._settings.placeholder || "";
	},
	...
	loadNext:function(count, start, callback, url, now){
		...
		if (config.datathrottle && !now){
			...
			this._throttle_request = webix.delay(function(){
				this.loadNext(count, start, callback, url);
...
webix.DataCollection = webix.proto({
	...
	refresh:function(){ this.callEvent("onBindRequest",[]); }
}, webix.ValidateCollection, webix.DataLoader, webix.EventSystem, webix.BaseBind, webix.Destruction, webix.Settings);
...
webix.Scrollable = {
	...
	scroll_setter:function(value){
		...
		var marker =  (value=="x"?"x":(value=="xy"?"xy":"y"));
...
webix.PagingAbility = {
	pager_setter:function(pager){
		if (typeof pager == "string"){
			...
			if (!ui_pager){
				...
				webix.delay(function(){
					...
					obj._settings.count = this.count();
					obj.refresh();
		...
		this.data.attachEvent("onStoreUpdated", function(){
			pager._settings.count = this.count();
			pager.refresh();
		}, null, true);
...
webix.Values = {
	...
	_clear_invalid:function(id,obj){
        if(id && this.elements[id] && this.elements[id].$view)
...
webix.TreeStore = {
	...
	remove:function(id){
		...
		var branch = this.branch[parentId];
		if (branch.length == 1 && parentId)
			delete this.branch[parentId];
		else
...
webix.TreeAPI = {
	...
	_tree_check_uncheck:function(id,mode){
		if(this._settings.threeState)
			return this._tree_check_uncheck_3(id,(arguments.length>1?mode:""));

		var item = this.getItem(id);

		item.checked = (arguments.length>1?mode:!item.checked);
		this.callEvent("onItemCheck", [id, item.checked]);
	},
...
webix.TreeStateCheckbox = {
	...
	_checkUncheckAll: function(id,mode){
...
webix.editors.combo = webix.extend({
	...
	showPopup:function(){
		var popup = webix.$$(this.config.suggest);
...
webix.editors.$popup = {
	...
	date:{
		...
		body:{ view:"calendar", borderless:true }
	},
	richselect:{
		view:"suggest",
...
webix.protoUI({
	...
	_config_table_from_file:function(){
		var cols = this._settings.columns;
		if (cols && cols.column)
			this._settings.columns = cols.column;
	},
	...
	_renderColumn:function(index,yr,force, single){
		...
		if (this._settings.scrollAlignY){
			if (yr[1] == (this.data.$pagesize || this.data.order.length)){
	...
	_restore_scroll_state:function(){
		if (this._x_scroll){
			var state = this.getScrollState();
			this._x_scroll._settings.scrollPos = this._y_scroll._settings.scrollPos = -1;
		...
		if (value == "order"){
			...
			control = {
				...
				$drag:webix.bind(function(s,e){
					...
					if (!id || !this.callEvent("onBeforeColumnDrag", [id.column, e])) return false;
					webix.DragControl._drag_context = { from:control, start:id, custom:"column_dnd" };
					return "<div class='webix_dd_drag_column'>"+(this.getColumnConfig(id.column).header[0].text||"&nbsp;")+"</div>";
				}, this),
...
webix.extend(webix.ui.datatable, {
	...
	_paint_spans_area:function(area, start, end){
		...
		for (var i = 0; i < count; i++) {
			...
			top += this.getItem(id).$height || this._settings.rowHeight;
		}
	},
	...
	_span_sum_height:function(start, end){
		...
		for (var i = start; i < end; i++){
			...
			summ += (next?next.$height:0) || this._settings.rowHeight;
		}
	...
	_add_span_to_area:function(area, ind, cind, config, top, start, id, cid){
		...
		var selected = config.$selected ? "webix_selected " : "";
		var span = webix.html.create("DIV", { 
			"column": cind,
			"row" : ind,
	...
	_adjust_spans_xy:function(){
		var state = this.getScrollState();
		for (var i=0; i<3; i++)
			this._spans_areas[i].style.top = "-"+state.y+"px";
		this._spans_areas[1].style.left = "-"+state.x+"px";
	}
...
webix.extend(webix.ui.datatable, {
	...
	_generate_menu_columns:function(){
		...
		var hidden = this.getState().hidden;
		for (var i = hidden.length - 1; i >= 0; i--)
			data.push({ id:hidden[i], value:this.getColumnConfig(hidden[i]).header[0].text, hidden:1 });
...
webix.ui.datafilter.richSelectFilter = webix.ui.datafilter.controlFilter = {
	...
	refresh:function(master, node, value){
		...
		if (value.options)
			...
			data = master.collectValues(value.columnId).slice(1);
...
webix.ui.datafilter.columnGroup = {
	...
	getHelper:function(node, config){
		return {
			open:function(){ config.closed = false; node.onclick(); },
			close:function(){ config.closed = true; node.onclick(); },
			isOpened:function(){ return config.closed; }
		};
	},
	refresh:function(master, node, config){
		function domark(batch, mode){
			master.eachColumn(function(id, col){
				if(col.batch){
					if(col.batch == batch){
						if (mode && master._hidden_column_hash[col.id])
							master.hideColumn(col.id, false, true);
						else if(!mode && !master._hidden_column_hash[col.id])
							master.hideColumn(col.id, true, true);
					}
				}
	        }, true);
			master._refresh_columns();
    	}
		...
		node.onclick = function(){
			...
			if (config.closed){
				...
				webix.delay(domark, this, [config.batch, true]);
			} else {
				config.closed = true;
				mark.className = "webix_tree_close";
				webix.delay(domark, this, [config.batch, false]);
			}
		};
		...
		if (!config.firstRun){
			...
			webix.delay(node.onclick, node);
		}
	},
	render:function(master, config){
		return "<div class='"+(config.closed?"webix_tree_close":"webix_tree_open")+"'></div>&nbsp;"+(config.groupText||""); 
	}
};
...
webix.protoUI({
	...
	_getColumnConfigSizes: function(date){ 
		...
		var containerHeight = this._content_height - this.config.headerHeight - 10 - (this._settings.timepicker?this.config.timepickerHeight:0);
	...
	_timepicker_template:function(date){
		return "<div><span class='webix_icon fa-clock-o'></span> "+this._settings.calendarTime(date)+"</div>";
	},
	...
	_update_zoom_level:function(date){
		...
		if (!this._zoom_size){
			...
			this._reserve_box_height = this._contentobj.offsetHeight - this.config.headerHeight - this.config.timepickerHeight;
...
webix.protoUI({
	...
	_suggest_config:function(value){
		...
		var suggest = { view:"multisuggest", separator:this.config.separator };
...
webix.protoUI({
	...
	defaults:{
		...
		body:{
			rows:[
				{ view:"list", autoheight:true, yCount:5, multiselect:"touch", select:true },
				{ view:"button", click:function(){
					var suggest = this.getParentView().getParentView();
					suggest.setMasterValue({ id:suggest.getList().getSelectedId(true).join(suggest.config.separator) });
					suggest.hide();
				}}
			]
		}
	...
	_set_on_popup_click:function(){
		...
		button._settings.value = this._settings.buttonText || webix.i18n.controls.select;
		button.refresh();
	},
	getButton:function(){
...
webix.protoUI({
	...
	type:{ 
		...
		templateEnd: function(obj, common, mark){
			return (obj.$template == "Separator"?"</div>":"</a>");
		},
	...
	getMenuItem:function(id){
		return this.getSubMenu(id).getItem(id);
	},
	...
	disableItem:function(id){
		this.getSubMenu(id).addCss(id, "webix_disabled");
	},
	enableItem:function(id){
		this.getSubMenu(id).removeCss(id, "webix_disabled");
	},
	...
	_create_sub_menu : function(data){
		...
		if(parentData && parentData.config){
			webix.extend(listConfig,
				parentData.config
			);
		}
    	var settings = this.getTopMenu()._settings.submenuConfig;
		if (settings)
			webix.extend(listConfig, settings);
...
webix.protoUI({
	...
	_getTabbarSizes: function(){
		var config = this.config,
			...
			totalWidth = this._input_width - config.tabOffset*2-(!config .type?(config.tabMargin)*(len-1):0)- len+1;
		if(totalWidth/len < config.tabMinWidth){
			return { max: (parseInt(totalWidth/config.tabMinWidth,10)||1)};
		}
		return {width: totalWidth/len};
	},
	_init_popup: function(){
		var obj = this._settings;
		if (!obj.tabbarPopup){
	...
	defaults:{
		...
		template:function(obj,common) {
			...
			if (!tabs.length){
				...
				contentWidth = common._input_width - obj.tabOffset*2-(!obj.type?(obj.tabMargin)*(tabs.length-1):0)- tabs.length+1;
				...
				for(i = 0; (i<tabs.length) && !lastTab; i++) {
					...
					if(sizes && sizes.max){
						...
						contentWidth = common._input_width - obj.tabOffset*2-(!obj.type&&(sizes.max>1)?(obj.tabMargin)*(sizes.max-1):0)- sizes.max+1;
		...
		while (first){
			...
			if (first.nodeType == 1){
				...
				if (name == "data"){
					...
					has_tags = 1;
					json[first.tagName] = webix.DataDriver.xml.tagToObject(first);
				}
			}
...
webix.protoUI({
	...
	cols_setter: function(cols){
		this._vertical_orientation = 0;
		this._initLayout.call(this,{cols: cols});
	},
	rows_setter: function(rows){
		this._vertical_orientation = 1;
		this._initLayout.call(this,{rows: rows});
	} ,
	_initLayout: function(cells){
		if(this._layout && this._layout.destructor)
			this._layout.destructor();

		this._layout = webix.ui._view(cells);
	...
	getActiveId:function(){
		return this._layout._cells[this._active_cell]._settings.id;
	},
...
webix.UploadDriver = {
	flash: {
		...
		_applyFlash: function(name,params){
...
webix.protoUI({
    ...
    _on_mouse_down_start:function(e){
        var trg = e.target || e.srcElement;
        if (trg.className == "webix_slider_handle")
            return this._start_handle_dnd.apply(this,arguments);
        else if (trg.className.indexOf("webix_slider") != -1){
            this.setValue(this._get_value_from_event.apply(this,arguments));
	...
	showProgress:function(config){
		...
		if (!this._progress){
			...
			this._progress = webix.html.create(
				"DIV",
				{ "class":"webix_progress_"+config.type,"style":this.getScrollState?("top:"+this.getScrollState().y +"px"):""},
...
webix.protoUI({
	...
	addSection:function(){
		var newone = this.getParentView().addView({
	...
	var webixCustomScroll = webix.CustomScroll = {
		...
		_enable_datatable:function(view){
			...
			view.attachEvent("onAfterRender", function(){
				...
				if (this._scrollTop > y)
				 	this._onscroll_y(y);
				else if (this._scrollLeft > x)
				 	this._onscroll_x(x);
